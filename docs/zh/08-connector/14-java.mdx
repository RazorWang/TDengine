...
2. java.lang.UnsatisfiedLinkError: no taos in java.library.path
   ...
   **解决方法**：Windows 下可以将 C:\TDengine\driver\taos.dll 拷贝到 C:\Windows\System32\ 目录下，Linux 下将建立如下软链 `ln -s /usr/local/taos/driver/libtaos.so.x.x.x.x /usr/lib/libtaos.so` 即可，macOS 先确认完成 Client 的安装，然后在 Java 扩展包文件夹建立库文件软连接 `ln -s /usr/local/lib/libtaos.dylib /Library/Java/Extensions/libtaos.dylib`。
